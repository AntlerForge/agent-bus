...
C4Container
  ...
  System_Boundary(boundary_project_agent_bus, "project:agent-bus") {
    ...
    Container(Component_amb_board, "AMB passive message board", "Component", "Does: persist AMB-only identities and passive note")
    ...
    ContainerDb(DataStore_amb_runtime_files, "AMB runtime files", "DataStore", "Human-readable AMB-only registry and inbox records")
    ...
    ContainerDb(DataStore_llm_selector_v3, "Knowledge Vault LLM selector v3", "DataStore", "Read-only mounted snapshot containing intrinsic mo")
  }
  Rel(Component_bus_core, DataStore_bus_runtime_files, "writes-to")
  Rel(Component_amb_board, DataStore_amb_runtime_files, "writes-to")
  Rel(Component_ha_notification_channel, DataStore_bus_runtime_files, "writes-to")
  Rel(Component_outcome_truth_sentinel, Component_bus_core, "depends-on")
