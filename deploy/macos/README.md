...
| `…-outcome-reporter` | `StartCalendarInterval` every 15 min at :02/:17/:32/:47 + `RunAtLoad`, no `KeepAlive` | Deliberately a short-lived scheduled job that publishes the read-only Mac snapshot to A6 and exits; the two-minute phase offset lets the ten-minute share-mount repair finish before sampling, and "not running" between runs is by design. |
