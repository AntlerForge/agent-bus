...
function selectorTarget(target) {
  ...
  return `<strong>${escapeHtml(model?.display_name || target.model_id)}</strong><small>${escapeHtml(surface?.display_name || target.surface_id)}${target.role ? ` · ${escapeHtml(formatStatus(target.role))}` : ""}</small>`;
}
function renderModels() {
  ...
  return `<div class="stats selector-stats"><div class="stat"><small>Selector state</small><strong class="status-word">${escapeHtml(selector.status)}</strong><div class="sub">schema ${escapeHtml(selector.schema_version)}</div></div><div class="stat"><small>Current models</small><strong>${selector.summary.model_count}</strong><div class="sub">validated entries</div></div><div class="stat"><small>Available surfaces</small><strong>${selector.summary.available_surface_count}</strong><div class="sub">Tony's current access</div></div><div class="stat"><small>Next review</small><strong class="date-word">${escapeHtml(selector.next_review)}</strong><div class="sub">weekly churn gate</div></div></div>
    ${selector.warnings.length ? `<div class="notice error">${selector.warnings.map(escapeHtml).join(" ")}</div>` : ""}
    <section class="panel"><div class="panel-header"><div><h2>Advisory routes</h2><p class="muted">These routes recommend a model and surface. They never dispatch work.</p></div>${selector.workflow_templates.length ? '<button class="button primary" id="new-workflow-button">Set up review panel</button>' : ""}</div><div class="route-grid">${routes}</div></section>
    <section class="panel surface-panel"><div class="panel-header"><div><h2>Access surfaces</h2><p class="muted">Availability is tracked separately from raw model capability.</p></div></div><div class="table-wrap"><table><thead><tr><th>Surface</th><th>Status</th><th>Provider</th><th>Version</th><th>Models</th></tr></thead><tbody>${selector.surfaces.map((surface) => `<tr><td><strong>${escapeHtml(surface.display_name)}</strong></td><td>${badge(surface.access)}</td><td>${escapeHtml(surface.provider)}</td><td>${escapeHtml(surface.installed_version || "Hosted")}</td><td>${escapeHtml((surface.models || []).map((id) => selector.models.find((model) => model.model_id === id)?.display_name || id).join(", "))}</td></tr>`).join("")}</tbody></table></div></section>`;
}
